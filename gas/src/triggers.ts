export function getTrigger(fName, atTime) {
/*  let triggers = ScriptAtt.getProjectTriggers();
  let found = triggers.find(tr => tr.getHandlerFunction() === fName);
  if (found) return found;
*/
  return ScriptApp.newTrigger(fName).timeBased().at(atTime).create();
}
 ...
 */
export function setupTriggerArguments(trigger, functionArguments, recurring) {
 ...
 */
export function handleTriggered(triggerUid) {
